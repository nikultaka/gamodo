...
export default function index({}) {
  ...
  const {
    ...
    refetch,
    remove,
    isLoading: popularNowDataLoading,
  } = useQuery(
    "popularNowData",
    async () => {
  ...
  return (
    <div className={styles.page_product_details}>
      ...
      <SingleHeader type={"popularnow"} />

      <div className="couponSlider moreGame">
        <Grid container spacing={2}>
          {!popularNowDataLoading ? (
            popularNowData?.game_list?.map((item, index) => {
              return (
                <Grid item sm={6} xs={6}>
      ...
      {popularNowData?.game_list?.length > 0 && (
        <Pagination
          style={{
            display: "flex",
